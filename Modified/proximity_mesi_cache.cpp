...
public:
    ...
    ProximityAwareSimulator(int dimX, int dimY) {
        int id = 0;
    ...
    void readAccess(int requesterId, uint64_t address) {
        ...
        // Limit number of sharers and evict farthest if needed
        const size_t maxSharers = 8;
        if (dirEntry.sharers.size() >= maxSharers) {
            int farthestId = -1;
            int maxDist = -1;
            ...
            if (farthestId != -1) {
                cores[farthestId].l2.invalidate(tag);
                dirEntry.sharers.erase(farthestId);
                invalidationMessages++;
            }
        }

        // Try cache-to-cache transfer from dirty owner (M state)
        ...
        // Try cache-to-cache from clean sharers (proximity-aware)
        int closestId = -1;
        int minDist = std::numeric_limits<int>::max();
        for (int sid : dirEntry.sharers) {
            if (sid == requesterId) continue;
            int dist = requester.distanceTo(cores[sid]);
            if (dist < minDist && cores[sid].l2.hasLine(tag)) {
                minDist = dist;
                closestId = sid;
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    ProximityAwareSimulator sim(4, 4);
